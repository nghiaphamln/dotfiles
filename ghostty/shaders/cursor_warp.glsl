...
const float THRESHOLD_MIN_DISTANCE = 0.6; // min distance to show trail (units of cursor height)
const float BLUR = 2.2; // blur size in pixels (for antialiasing)
const float TRAIL_THICKNESS = 1.08;  // 1.0 = full cursor height, 0.0 = zero height, >1.0 = funky aah
const float TRAIL_THICKNESS_X = 1.12;

const float FADE_ENABLED = 1.0; // 1.0 to enable fade gradient along the trail, 0.0 to disable
const float FADE_EXPONENT = 2.8; // exponent for fade gradient along the trail
