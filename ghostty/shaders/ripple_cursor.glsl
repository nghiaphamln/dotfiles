...
const float RING_THICKNESS = 0.035;        // Ring width in normalized coords
const float CURSOR_WIDTH_CHANGE_THRESHOLD = 0.2; // Triggers ripple if cursor width changes by this fraction
vec4 COLOR = vec4(0.83, 0.65, 0.18, 0.92); // change to iCurrentCursorColor for your cursor's color
const float BLUR = 5.5;                    // Blur level in pixels
const float ANIMATION_START_OFFSET = 0.04; // Start the ripple slightly progressed (0.0 - 1.0)
